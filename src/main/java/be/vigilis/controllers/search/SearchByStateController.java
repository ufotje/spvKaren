...
import be.vigilis.repositories.InvoiceRepo;
...
@Component
public class SearchByStateController {
    @FXML
    ...
    @Autowired
    private GeneralRepo gRepo;

    @FXML
    private void searchState(){
        ObservableList<General> generals = observableArrayList();
        ObservableList<Invoices> invoices = observableArrayList(repo.findByState(state.getText()));
        ObservableList<Address> addresses = observableArrayList();
        if(!invoices.isEmpty()){
            for(Invoices i : invoices){
                General g = gRepo.findByInvoice(i);
                generals.add(g);
                addresses.add(g.getAddress());
            }
            MenuController.stage.close();
            ChangeScene.init("/fxml/search/views/searchByState.fxml", "Bedrijven wiens aanvraag " + state.getText() + " is");
            ...
            Warning.alert("Record Not found!", "Er werden geen resultaten gevonden.");
        }
    }
}
